...
void traceW(int h, int j, ps_t& ps, ps_stack_t& gstack) {
								for (int i = 1; i < j-TURN; ++i) {
																...
																int d3 = (i>1)?Ed3(j,i,i-1):0;
																int d5 = (j<length)?Ed5(j,i,j+1):0;
																
																int Wij = V(i,j) + auPenalty(i, j) + d3 + d5 + wim1;
																if (Wij + ps.total() <= mfe + delta ) {
																								ps_t ps1(ps);
																								ps1.push(segment(i, j, lV, V(i,j)));
																								if (wim1 == W[i-1]) ps1.push(segment(1, i-1, lW, W[i-1]));
																								ps1.accumulate(auPenalty(i, j) + d3 + d5);
																								push_to_gstack(gstack, ps1);
																}
								}
								...
								if (W[j-1] + ps.total() <= mfe + delta) {
																ps_t ps1(ps);
																ps1.push(segment(1, j-1, lW, V(1,j-1)));
																push_to_gstack(gstack, ps1);
								}
}
