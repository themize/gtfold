...
static int load_constraints(const char* constr_file, int verbose=0) {
	...
	map<int, pair<int,int> > all_bases;
	for(it=0; it< nFBP; it++) {
		for(int k=1;k<= FBP[it][2];k++) {
			pair<int,int> base_pair(FBP[it][0]+k-1, FBP[it][1]-k+1);
			pair<map<int,pair<int,int> >::iterator, bool> retVal;
			retVal = all_bases.insert(pair<int, pair<int,int> >(base_pair.first, base_pair));
			if (retVal.second == false) {
				fprintf(stderr, "\nDuplicate base %d encountered in Constraints. Base Pair %d,%d\n",
					 base_pair.first, base_pair.first, base_pair.second);
				exit(1);
			}
			retVal = all_bases.insert(pair<int, pair<int,int> >(base_pair.second, base_pair) );
			if (retVal.second == false) {
				fprintf(stderr, "\nDuplicate base %d encountered in Constraints. Base Pair %d,%d\n",
					 base_pair.first, base_pair.first, base_pair.second);
				exit(1);
	...
	vector<pair<int,int> > verify_stack;
	for (map<int,pair<int,int> >::iterator it = all_bases.begin(); it != all_bases.end(); ++it) {

		pair<int, pair<int,int> > map_element = (*it);
		int base = map_element.first;
		pair<int, int> base_pair = map_element.second;
		if (base_pair.first == base) {
			verify_stack.push_back(base_pair);
		} else {
        		pair<int,int> last_base_pair = verify_stack.back();
